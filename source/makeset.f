...
!    x   		(PTYPE(II,JF),II=1,6),',',KPA,KPB
           ...
           if(KPA /= KPB) then          ! how to deal with this case:
            ...
            if(listcc>0) write(6,4801) MIXPOT,PTYPE(1,JF),KPA,KPB,OP
...
C			Normal KK=0 should exclude off-diagonal monopoles
       ...
       ENDIF
C
        if (JFTT.eq.10.and.BAND(2,ic,ia).ne.BAND(2,ic,ib) .or.
     x      JFTT.eq.11.and.BAND(1,ic,ia).ne.BAND(1,ic,ib) ) go to 300
